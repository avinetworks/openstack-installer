...
git checkout newton-eol -b newton
...
apt-get install barbican-api -y
...
sed -i "s/db_auto_create = False/db_auto_create = True/g" /etc/barbican/barbican.conf
su -s /bin/sh -c "barbican-db-manage clean" barbican
