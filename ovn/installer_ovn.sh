...
apt-get -y install neutron-server neutron-plugin-ml2 ovn-central ovn-host ovn-docker ovn-common neutron-dhcp-agent neutron-metadata-agent neutron-l3-agent
apt install -y python3-pip
pip3 install networking-ovn

/usr/share/openvswitch/scripts/ovs-ctl start  --system-id="random"
ovn-nbctl set-connection ptcp:6641:$my_ip -- 
ovn-sbctl set-connection ptcp:6642:$my_ip --
...
sed -i s/OVERLAY_INTERFACE_IP_ADDRESS/$my_ip/g /etc/neutron/plugins/ml2/ml2_conf.ini
...
/usr/share/openvswitch/scripts/ovs-ctl start --system-id="random"
ovs-vsctl set open . external-ids:ovn-remote=tcp:$my_ip:6642
...
ovs-vsctl --may-exist add-port br-provider ens4
